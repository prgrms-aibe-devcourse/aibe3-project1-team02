...
export default function Destination() {
    ...
    useEffect(() => {
        const fetchReviews = async () => {
            const { data, error } = await supabase.from('destination').select('id, reviews_count')
            ...
            const updated = destinations.map((dest) => {
                const matched = data.find((d) => d.id === dest.id)
                return {
                    ...dest,
                    reviews: matched?.reviews_count ?? dest.reviews,
                }
            })
