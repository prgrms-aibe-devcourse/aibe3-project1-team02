...
export default function MyPlansPage() {
    ...
    const filteredPlans = activeTab === 'all' ? mockPlans : mockPlans.filter((plan) => plan.status === activeTab)
                    ...
                    <div className="flex space-x-1 bg-white rounded-lg p-1 shadow-sm">
                        <button
                            ...
                            className={`px-4 py-2 rounded-md transition-colors cursor-pointer whitespace-nowrap ${
                                activeTab === 'all' ? 'bg-blue-600 text-white' : 'text-gray-600 hover:text-blue-600'
                            }`}
                        >
                            전체 ({mockPlans.length})
                        </button>
                ...
                <div className="mt-16 grid grid-cols-1 md:grid-cols-4 gap-6">
                    ...
                    <div className="bg-white rounded-xl p-6 text-center">
                        <h4 className="text-2xl font-bold text-purple-600 mb-2">
                            {mockPlans.reduce((acc, plan) => acc + plan.travelers, 0)}
                        </h4>
